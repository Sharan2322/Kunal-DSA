package com.kunal;

public class RotatedBS {
	public static void main(String[] args) {

		int[] arr = {4,5,6,7,0,1,2};
		int target = 0;
		int pivot = findPivot(arr);
		int fistHalf = BinarySearch(arr, target, 0, pivot);

		if (fistHalf != -1){
			System.out.println(fistHalf);
		}
		else{
			System.out.println(BinarySearch(arr, target, pivot +1 , arr.length - 1));
		}

	}


	public int search(int[] nums, int target) {
		int pivot = findPivot(nums);
		int fistHalf = BinarySearch(nums, target, 0, pivot);
		if (fistHalf != -1){
			return fistHalf;
		}
		else{
		 return BinarySearch(nums, target, pivot +1 , nums.length);
		}
	}

	static int search2(int[] nums, int target){
		int pivot = findPivot(nums);
		// if  pivot is not found, it means the array is not rotated
		if (pivot == -1){
			return BinarySearch(nums, target, 0, nums.length - 1);
		}
		// if pivot is found, it means array is rotated, you have found 2 asc sorted arrays
		if (nums[pivot] == target){
			return pivot;
		}
		if (target >= nums[0]){
			// first half, before pivot
			return BinarySearch(nums, target, 0, pivot - 1);
		}
		// second half, after pivot
		return BinarySearch(nums, target, pivot + 1, nums.length - 1);
	}


	// this will not work in duplicate values
	static int findPivot(int[] arr){
		int start = 0;
		int end = arr.length -1;
		int ans = -1;
		while (start <= end){
			int mid = start + (end - start)/2;
			// 4 case
			if ( mid < end && arr[mid] > arr[mid + 1]){
				return ans = mid;
			}
			if ( mid > start && arr[mid] < arr[mid - 1] ){
				return ans = mid - 1;
			}
			if (arr[mid] <= arr[start]){
				end = mid -1;
			}
			else {
				start = mid + 1;
			}
		}
		return ans;
	}




	// use this for duplicate values
	static int findPivotWithDuplicates(int[] arr){
		int start = 0;
		int end = arr.length -1;
		while (start <= end) {
			int mid = start + (end - start) / 2;
			if (mid < end && arr[mid] > arr[mid + 1]) {
				return mid;
			}
			if (mid > start && arr[mid] < arr[mid - 1]) {
				return mid - 1;
			}
			// if elements at middle, start, end are equal then just skip the duplicates
			if(arr[mid] == arr[start] && arr[mid] == arr[end]){
				// skip the duplicates
				// check whether start is pivot
				if (arr[start] > arr[start + 1]){
					return start;
				}
				start++;
				// check whether end is pivot
				if (arr[end] < arr[end - 1]){
					return  end - 1;
				}
				end--;
			}
			// left side is sorted, so pivot should be in right
			else if (arr[start] < arr[mid] || arr[start] == arr[mid] && arr[mid] > arr[end]){
				start = mid + 1;
			}
			else {
				end = mid - 1;
			}
		}
		return -1;
	}





	static int BinarySearch(int[] arr, int target, int start, int end){
		while (start <= end){
			int mid = start + (end - start)/2;

			if (target < arr[mid]){
				end = mid - 1;
			}
			else if (target > arr[mid]){
				start = mid + 1;
			}
			else {
				return mid;
			}
		}
		return -1;
	}


}
